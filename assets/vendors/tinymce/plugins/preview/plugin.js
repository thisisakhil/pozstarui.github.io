...
(function () {
  ...
  const getPreviewHtml = editor => {
    ...
    const previewHtml = '<!DOCTYPE html>' + '<html>' + '<head>' + headHtml + '<style>
      .table_60 {
        width: 60%;
  }
...
</head > ' + ' < body id = "' + encode(bodyId) + '" class="mce-content-body ' + encode(bodyClass) + '"' + dirAttr + ' > ' + editor.getContent() + preventClicksOnLinksScript + '</body > ' + '</html > ';
  return previewHtml;
};
