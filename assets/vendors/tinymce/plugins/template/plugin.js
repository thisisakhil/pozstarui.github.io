...
(function () {
  ...
  const getPreviewContent = (editor, html) => {
    ...
    if (html.indexOf('<html>') === -1) {
      ...
      html = '<!DOCTYPE html>' + '<html>' + '<head>' + '<base href="' + encode(editor.documentBaseURI.getURI()) + '">' + contentCssEntries + preventClicksOnLinksScript + '<style>
        .table_60 {
        width: 60 %;
      }
...
</head > ' + ' < body class="' + encode(bodyClass) + '"' + dirAttr + ' > ' + html + '</body > ' + '</html > ';
      }
